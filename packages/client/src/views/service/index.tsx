...
const Service = () => {
  ...
  const columns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      key: "created_at",
      render:(_, data: any) => {
        return <span>{dayjs(data.created_at).format('YY-MM-DD HH:mm:ss')}</span>
      }
    },
    {
      title: "状态",
    ...
    },
    {
      ...
      key: 'memory',
      render: (_, data: any) => {
        return <Space size="small">{Math.floor(data.monit.memory/1048576)}M</Space>;
      },
    },
    {
      title: "用户",
    ...
    },
    {
      ...
      render: (_, data: any) => {
        return (
          <Space size="small">
            <Button type="primary" size="small" danger onClick={() => stopServiceHandle(data.pm_id)}>
              暂停
            </Button>
            <Button type="primary" size="small" onClick={() => startServiceHandle(data.pm_id)}>
              恢复
            </Button>
          </Space>
        );
